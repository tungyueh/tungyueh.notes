...
### Backtrack
* 當 refactoring 覺得很順利所以沒有每一步都跑過測試最後改太多測試才發現錯了，這時候代表已經失去控制
* 失去控制的時候不該繼續往前要回到正確的地方開始慢慢來，每次改變都要測試
* 當進入狀態最難的是停下來倒回去，重新來過只需要花一點時間，如果繼續前進可能會花更多時間 debug
### Duets
* 有同伴可以幫助我們有規律的做好每一步，都同伴不懂在做什麼那就是出錯的信號
* 解釋即將進行的動作可以幫助我們踏上正確的韻律來 refactoring
* 看到新的可能性會忍不住去把問題都解決但不應該完全停下來清理之前的技術債，通常 big refactoring 都是一場災難
* 當需要增加新功能可以先把原本的地方清理乾淨，先接觸 code 可以讓我們更理解內容讓之後增加新功能更快速
* Refactoring 時候不該思考 code 是否正確不然會擾亂思緒，可以先記下來之後需要變動的部分
